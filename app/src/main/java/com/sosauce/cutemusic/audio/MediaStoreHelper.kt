...
object MediaStoreHelper {
        ...
        withContext(Dispatchers.IO) {
            ...
            contentResolver.query(
                ...
                while (cursor.moveToNext()) {
                    ...
                    val albumInfo = Album(id, album, artist, numberOfSongs, albumArt, emptyList())
                    albums.add(albumInfo)
                }
            }

            for (album in albums) {
                album.songs = getMusics(contentResolver, albumId = album.id)
                album.albumArt = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
    ...
    suspend fun getArtists(contentResolver: ContentResolver): List<Artist> =
        withContext(Dispatchers.IO) {
            ...
            val allAlbums = getAlbums(contentResolver)
            val allMusics = getMusics(contentResolver)

            for (artist in artists) {
                artist.albums = allAlbums.filter { it.artist == artist.name }
                artist.songs = allMusics.filter { it.artist == artist.name }
            }
